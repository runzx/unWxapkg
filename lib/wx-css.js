...
class WxCss extends Base {
  constructor({ dir, mainDir, root = '', filename = '', moreInfo = false } = {}) {
    ...
    this.actualPure = {}
    this.importCnt = {}
    this.frameFile = "" // "page-frame.html", "app-wxss.js", "page-frame.js"
    this.blockCss = []
    this.commonStyle = {}
    this.onlyTest = false
    this.saveDir = dir
  ...
  start() {
    ...
    this.runOnce()
    this.onlyTest = false
    console.log("Import count info: %o", this.importCnt)
    this.preCss()
  ...
  saveFile() {
    ...
    for (let name in result) {
      let pathFile = this.getPathName(this.changeExt(name, ".wxss"), dir)
  ...
  preRun(files) {
    for (const name of files) {
      ...
      if (code.includes("setCssToHead(")) {
        let lastName = name
        let dirSplit = name.split(this.dir + this.pathSep)
        if (dirSplit.length > 1) {
          lastName = this.getPathName(dirSplit[1], this.saveDir)
        }
...
${scriptCode}`
  ...
  runVM(cssFile, code) {
    ...
    Object.keys(wxAppCode).forEach(fileName => {
      cssFile = this.getPathName(fileName, dir)
      fileName.endsWith(".wxss") && wxAppCode[fileName]()
    })
  ...
  cssRebuild(data, cssFile) {
    ...
    const makeup = (data) => {
      ...
      if (onlyTest) {
        ...
        if (!actualPure[data] && !blockCss.includes(this.changeExt(this.toDir(cssFile, this.frameFile), ""))) {
      ...
      if (isPure && actualPure[data] != cssFile) {
        if (actualPure[data]) return '@import "' + this.changeExt(this.toDir(actualPure[data], cssFile), ".wxss") + '";\n'
        else {
