...
class WxCss extends Base {
  constructor(dir, subPack) {
    ...
    this.actualPure = {}
    this.importCnt = {}
    this.frameName = "" // "page-frame.html", "app-wxss.js", "page-frame.js"
    this.blockCss = []
    this.commonStyle = {}
    this.onlyTest = false
    this.saveDir = dir
  ...
  getMainCss() {
    const frameFile = ["page-frame.html", "app-wxss.js", "page-frame.js"].find(i =>
      this.isExistFile(i))
    this.frameName = this.getPathName(frameFile)
    if (!frameFile) throw new Error('page-frame-like file is not found!')

    let code = this.getFileContent(this.frameName)
  ...
  cssRebuild(data, cssFile) {
    ...
    const makeup = (data) => {
      ...
      if (onlyTest) {
        ...
        if (!actualPure[data] && !blockCss.includes(this.changeExt(this.toDir(cssFile, this.frameName), ""))) {
