...
class WxZ extends Base {
  ...
  catchZ(code) {
    ...
    this.vmRun(code, { sandbox: { z, debugInfo: [] } })
    return z
  }
  catchZGroup(code, groupPreStr) {
    ...
    for (let preStr of groupPreStr) {
      ...
      this.vmRun(content, { sandbox: { z, debugInfo: [] } })
  ...
  restoreAll(z) {
    ...
    const ans = []
    for (let e of z) ans.push(this.restoreSingle(e, false))
    return ans
  }
  restoreGroup(z) {
    const ans = []
    for (let g in z.mul) {
      const singleAns = []
      for (let e of z.mul[g]) singleAns.push(this.restoreSingle(e, false))
      ans[g] = singleAns
    }
  ...
  restoreSingle(ops, withScope = false) {
    ...
    const enBrace = (value, type = '{') => {
      ...
      if (!res) throw Error("Unknown brace type " + type)
      return res
    }
    function _enBrace(value, type = '{') {
    ...
    const restoreNext = (ops, w = withScope) => {

      return self.restoreSingle(ops, w)
    }
    const jsoToWxon = obj => {
      let ans = ""
      ...
      const action = actions.get(this.type(obj)) || actions.get('default')
      return action()
    }
    function _jsoToWxon(obj) {//convert JS Object to Wechat Object Notation(No quotes@key+str)
      let ans = ""
