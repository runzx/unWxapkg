...
class Wxapkg extends Base {
  ...
  doConfig() {
    console.log('\nstart config file ...\n',)
    const cfg = WxCfg.init({ dir: this.dir })
    cfg.start()
  }
  doCss() {
    const root = this.getSubRoot()
    const dir = root ? this.getPathName(root) : this.dir
    const cfg = WxCss.init({
      dir, mainDir: this.dir, root,
  ...
  doJs() {
    ...
    if (!this.isExistFile(filename, dir)) throw new Error('app-service.js file is not found!')
    const cfg = WxJs.init({
      dir, mainDir: this.dir, root,
      filename, moreInfo: this.moreInfo
    })
  ...
  getSubRoot() {
    ...
    workDir = (workDir.startsWith('/') ? '.' : '') + workDir
    // workDir = this.getPathName(workDir)
    return workDir
  }
