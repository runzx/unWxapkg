...
class Wxml extends WxZ {
  ...
  start() {
    // const root = this.getFileName(this.dir)
    this.subPack && console.log('\nsubPackages.root: %s', this.subPack)
  ...
  saveWxs(f_, requireInfo) {
    ...
    const pF = {}
    const wxsList = {}
    const files = Object.keys(f_).filter(key => typeof f_[key] === 'function')
    console.log('Generate wxs: %d', files.length)
    files.forEach(info => {
        ...
        Object.keys(now).forEach(deps => {
          ...
          if (ref.includes(":"))
            res.push("<wxs module=\"" + deps + "\">\n" +
              this.fmtWxs(requireInfo[ref].toString()) + "\n</wxs>")
  ...
  tryWxml(z, d_, e_, wxsList, xPool) {
    ...
    console.log("Decompile wxml files: ", files.length)
    files.forEach(name => {
      let state = [null]
      const rDs = d_[name]
      ...
      try {
        ...
        if (state[0] === null) this.save(this.getPathName(name + ".ori.js", dir), code)
        else this.save(this.getPathName(name + ".tem-" + state[0] + ".ori.js", dir),
  ...
  doWxml(state, name, code, z, xPool, rDs, wxsList, moreInfo = this.moreInfo) {
    ...
    name = this.getPathName(name, dir)
